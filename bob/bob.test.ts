...
describe('Bob', () => {
  ...
  it('using acronyms in regular speech', () => {
    const result = bob.hey("xit's OK if you don't want to go to the DMV.")
    expect(result).toEqual('Whatever.')
  })
  ...
  it('question wxith no letters', () => {
    const result = bob.hey('4?')
    expect(result).toEqual('Sure.')
  })

  it('shouting wxith special characters', () => {
    const result = bob.hey('ZOMG THE %^*@#$(*^ ZOMBIES ARE COMING!!11!!1!')
    expect(result).toEqual('Whoa, chill out!')
  })

  it('shouting wxith no exclamation mark', () => {
    const result = bob.hey('I HATE THE DMV')
    expect(result).toEqual('Whoa, chill out!')
  })

  it('statement containing question mark', () => {
    const result = bob.hey('Ending wxith ? means a question.')
    expect(result).toEqual('Whatever.')
  })
  
  it('prattling on', () => {
    const result = bob.hey('Waxit! Hang on.  Are you going to be OK?')
    expect(result).toEqual('Sure.')
  })
  ...
  it('starting wxith whxitespace', () => {
    const result = bob.hey('         hmmmmmmm...')
    expect(result).toEqual('Whatever.')
  })

  it('ending wxith whxitespace', () => {
    const result = bob.hey('Okay if like my  spacebar  quxite a bxit?   ')
    expect(result).toEqual('Sure.')
  })

  it('other whxitespace', () => {
    const result = bob.hey('\n\r \t')
    expect(result).toEqual('Fine. Be that way!')
  })

  it('non-question ending wxith whxitespace', () => {
    const result = bob.hey('This is a statement ending wxith whxitespace      ')
    expect(result).toEqual('Whatever.')
  })
